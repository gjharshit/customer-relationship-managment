...
<head>
      ...
      <title>Download Yashasvi Files</title>
...
<?php
    ...
    if ($handle = opendir('yashasvi_uploads/po_uploads')) {
...
<?php
    ...
    if ($handle = opendir('yashasvi_uploads/quote_uploads')) {
        while (false !== ($entry = readdir($handle))) {
            if ($entry != "." && $entry != "..") {
                $quote_files[] = $entry;
            }
...
<?php
	...
	echo "<h1>The list of all the files in the Yashasvi Downloads Directory:</h1>";
	echo "<h2>Click on the links below to download the files!</h2>";

	// Output the files in the Cusotmer PO Uploads folder
	echo "<h3>Click on the links below to download the Yashasvi PO Documents!</h3>";
	echo "<table width='75%'>";
	echo "<th width=\"30%\" style=\"color: red;\">Disti Invoice Number:</th><th style=\"color: red;\">Name of the file:</th><th width=\"30%\" style=\"color: red;\">Disti Name:</th>";

	foreach ($po_files as $value) {
		$disti_inv_db = "SELECT disti_inv_number FROM yashasvi_attachments WHERE ";
		$disti_inv_db .= "yashasvi_po LIKE '$value'";

		$dist_name_db = "SELECT dist_name FROM yashasvi_attachments WHERE ";
		$dist_name_db .= "yashasvi_po LIKE '$value'";

		$result1 = mysqli_query($connection, $disti_inv_db);
		$result2 = mysqli_query($connection, $dist_name_db);

		$disti_invoice = mysqli_fetch_array($result1, MYSQLI_ASSOC);
		$disti_name = mysqli_fetch_array($result2, MYSQLI_ASSOC);

		echo "<tr><td>";
		echo $disti_invoice['disti_inv_number'];
		echo "</td>";
		echo "<td>";
		echo "<a href=\"download_po_yashasvi.php?link=$value\">$value</a>";
		echo "</td>";
		echo "<td>";
		echo $disti_name['dist_name'];
		echo "</td>";
		echo "<tr>";
	...
	echo "<h3>Click on the links below to download the Yashasvi Quote Documents!</h3>";
	echo "<table width='75%'>";
	echo "<th width=\"30%\" style=\"color: red;\">Disti Invoice Number:</th><th style=\"color: red;\">Name of the file:</th><th width=\"30%\" style=\"color: red;\">Disti Name:</th>";

	foreach ($quote_files as $value) {
        $disti_inv_db = "SELECT disti_inv_number FROM yashasvi_attachments WHERE ";
		$disti_inv_db .= "yashasvi_quote LIKE '$value'";

		$dist_name_db = "SELECT dist_name FROM yashasvi_attachments WHERE ";
		$dist_name_db .= "yashasvi_quote LIKE '$value'";
		...
		echo "</td>";
		echo "<td>";
		echo "<a href=\"download_quote_yashasvi.php?link=$value\">$value</a>";
		echo "</td>";
		echo "<td>";
		echo $disti_name['dist_name'];
		echo "</td>";
